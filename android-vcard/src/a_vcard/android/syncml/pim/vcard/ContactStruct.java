...
public class ContactStruct {
    ...
    /** Only for GET. Use addAddress() to PUT. */
    ...
    /**
     * Add a Address info to addressList.
     */
    public void addAddress(int type, String formatted, String poBox, String extended,
            String street, String city, String state, String postal, String country, String label,
            boolean isPrimary) {
        if (addressList == null) {
            addressList = new ArrayList<Address>();
        }
        Address address = new Address(type, formatted, poBox, extended, street, city, state,
                postal, country, label, isPrimary);
        addressList.add(address);
    }

    private void addAddress(int type, String formatted, String label, boolean isPrimary) {
        if (addressList == null) {
            addressList = new ArrayList<Address>();
        }
        Address address = new Address(type, formatted, label, isPrimary);
        addressList.add(address);
    }
    ...
    public static ContactStruct constructContactFromVNode(VNode node,
        ...
        for (PropertyNode propertyNode: node.propList) {
            ...
            if (name.equals("VERSION")) {
                ...
                if (propertyNode.propValue_vector.size() == 1) {
                    nameFromNProperty = new Name(getElementAttempt(propertyNode, 0));
                } else {
                    nameFromNProperty = new Name(getElementAttempt(propertyNode, 0),
                            getElementAttempt(propertyNode, 1), getElementAttempt(propertyNode, 2),
                            getElementAttempt(propertyNode, 3), getElementAttempt(propertyNode, 4),
                            nameOrderType);
                }
                ...
                //              ; Code, Country Name
                if (propertyNode.propValue_vector.size() == 1) {
                    contact.addAddress(type, getElementAttempt(propertyNode, 0), label, isPrimary);
                } else {
                    contact.addAddress(type, null, getElementAttempt(propertyNode, 0),
                            getElementAttempt(propertyNode, 1), getElementAttempt(propertyNode, 2),
                            getElementAttempt(propertyNode, 3), getElementAttempt(propertyNode, 4),
                            getElementAttempt(propertyNode, 5), getElementAttempt(propertyNode, 6),
                            label, isPrimary);
                }
				...
				for (String typeString : propertyNode.paramMap_TYPE) {
					if (typeString.equals("PREF") && !prefIsSetAddress) {
						...
						// exporters emit non-standard types. We do not handle their
						// values now.
						type = Contacts.ContactMethodsColumns.TYPE_CUSTOM;
						label = typeString;
					}
                ...
                // Assume IMPP is stored in BASE64. In that case,
				...
				contact.addContactmethod(kind, type, propertyNode.propValue,
						label, isPrimary);
                ...
                boolean isPrimary = false;
                for (String typeString : propertyNode.paramMap_TYPE) {
                    if (typeString.equals("PREF") && !prefIsSetUrl) {
                        // Only first "PREF" is considered.
                        prefIsSetUrl = true;
                        isPrimary = true;
                    } else if (typeString.equalsIgnoreCase("HOME")) {
        ...
        if (fullName != null) {
            contact.name = new Name(fullName);
        ...
        if (contact.phoneticName == null &&
            ...
            String first;
            String second;
            if (nameOrderType == Name.NAME_ORDER_TYPE_JAPANESE) {
        ...
        if (!prefIsSetUrl && contact.contactmethodList != null) {
            for (ContactMethod contactMethod : contact.contactmethodList) {
                if (contactMethod.kind == Contacts.KIND_URL) {
                    contactMethod.isPrimary = true;
                    break;
                }
            }
    ...
    public static String getElementAttempt(PropertyNode node, int index) {
        if (node.propValue_vector.size() > index) {
            return node.propValue_vector.get(index);
        } else {
            return null;
        }
    }
...
//    }
    ...
    public boolean isIgnorable() {
        return TextUtils.isEmpty(name.toString()) &&
